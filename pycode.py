...
def tsp_hk(distance_matrix):
    def tsp_hk_rec(cities, start, cache):
        if len(cities) == 1:  # If there is only one city left
            return distance_matrix[start][cities[0]]  # We return the distance from the start to that city

        else:  # If there is more than one city, we need to make a list of cities we need to visit and find the shortest path
            key = str(cities) + str(start)  # Make a key
            if key in cache:  # Check our cache first
                return cache[key]

            min_length_tour = float('inf')  # Make a variable to hold the minimum distance
            new_cities = []
            for city in cities:  # Make our new list of cities to visit
                if city != start:
                    new_cities.append(city)

            for new_city in new_cities:  # Iterate through our list of new cities to find the shortest path
                current_tour_length = tsp_hk_rec(new_cities, new_city, cache) + distance_matrix[start][new_city]  # Recursively call our function to find the minimum length and then add the distance from the start to get the total length
                min_length_tour = min(min_length_tour, current_tour_length)  # Set the overall minimum length tour

            cache[key] = min_length_tour  # Add to our cache
            return min_length_tour  # Return the smallest tour

    number_of_cities = len(distance_matrix)  # Get the number of cities

    if number_of_cities == 0 or len(distance_matrix[0]) == 0:  # Check for empty matrix or a matrix with no cities
        return 0

    shortest_length_tour = float('inf')  # Use this to find the overall shortest path
    cache = {}  # Make our cache for memoization

    for start_city in range(number_of_cities):  # Iterate through all possible start cities and find the shortest path out of them
        cities = list(range(number_of_cities))  # Make a list of our cities to visit
        current_tour_length = tsp_hk_rec(cities, start_city, cache)  # Find the tour length for this specific start city
        shortest_length_tour = min(shortest_length_tour, current_tour_length)  # Find the shortest tour out of all possible cities

    return shortest_length_tour  # Return our overall shortest tour

def generate_distance_matrix(num_cities, max_distance=100):
...
def printMatrix():
    for i in range(10,20):
        print(generate_distance_matrix(i))

def timing():
    print('Running Held Karp')
    matrix1 = [[0, 72, 5, 62, 70, 55, 22, 26, 15, 16], [55, 0, 12, 70, 24, 91, 32, 53, 96, 46], [23, 85, 0, 71, 8, 83, 98, 41, 97, 44], [91, 42, 11, 0, 54, 63, 33, 58, 35, 45], [56, 86, 23, 65, 0, 89, 70, 95, 40, 70], [31, 72, 83, 70, 86, 0, 90, 4, 58, 22], [87, 1, 64, 82, 98, 7, 0, 8, 88, 82], [85, 3, 93, 6, 16, 20, 61, 0, 6, 50], [31, 30, 88, 33, 72, 89, 20, 83, 0, 42], [65, 6, 43, 76, 64, 4, 45, 75, 15, 0]]
    matrix2 = [[0, 43, 100, 99, 72, 15, 58, 98, 45, 53, 92], [98, 0, 83, 19, 77, 28, 24, 67, 80, 46, 78], [75, 16, 0, 66, 41, 6, 73, 83, 68, 24, 6], [37, 59, 25, 0, 29, 58, 52, 80, 21, 45, 4], [12, 61, 25, 43, 0, 46, 86, 25, 100, 34, 2], [14, 65, 17, 62, 62, 0, 51, 85, 45, 88, 31], [19, 6, 4, 88, 28, 57, 0, 46, 92, 91, 92], [55, 40, 57, 20, 60, 24, 93, 0, 100, 68, 99], [67, 48, 61, 47, 20, 72, 31, 36, 0, 49, 85], [35, 55, 64, 61, 30, 50, 74, 17, 66, 0, 25], [16, 29, 6, 78, 56, 9, 95, 77, 41, 70, 0]]
    ...
    matrixList = [matrix1, matrix2, matrix3, matrix4, matrix5, matrix6, matrix7, matrix8, matrix9, matrix10]

    for i in range(0, 10):
        avg = 0
        for j in range(0, 10):
            start_hk = time.time()
            answer = tsp_hk(matrixList[i])
            end_hk = time.time()
            avg += (end_hk-start_hk)
        print(f"{i+10} cities in {(avg/10):.8f} seconds on average")
